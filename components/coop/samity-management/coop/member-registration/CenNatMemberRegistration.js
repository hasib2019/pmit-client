...
const CenNatMemberRegistration = () => {
  ...
  const getArray = (allRespData, refSamityId) => {
    if (allRespData) {
      ...
      newlyRemainingIdArray = allRespData.filter((element) => {
        if (refSamityId.indexOf(element.id) == -1) {
          return element;
        }
      });
  ...
  const getCenNatSamityAllData = async () => {
    try {
      ...
      if (allRespDataShow != undefined) {
        const refSamityId = responseShow.data.data.map((elem) => elem.refSamityId);
  ...
  const onEdit = async (
    ...
    setSamityInfo({
      id: found.id,
      samityId: refSamityId,
      samityCode: samityCode,
      samityName: memberName,
      samityRegistrationDate: dateFormat(samityRegistrationDate),
      memberNameBangla: samitySignatoryPerson,
      samityDetailsAddress: samityDetailsAddress,
    });
    setDataEmpty(false);
      ...
      <Grid container className="section" spacing={1.8}>
        ...
        <Grid item lg={4} md={4} xl={4} xs={12}>
          <TextField
            ...
            // onChange={handleChangeSamityName}
            required
            value={samityInfo.memberNameBangla}
            variant="outlined"
            size="small"
          ></TextField>
        </Grid>
              ...
              <TableHead className="table-head">
                <TableRow>
                  ...
                  <StyledTableCell sx={{ whiteSpace: 'initial !important' }}>
                    কেন্দ্রীয়/জাতীয় সমিতির পক্ষে স্বাক্ষরের ব্যক্তির নাম
                  </StyledTableCell>
                      ...
                      <Tooltip title="সম্পাদন করুন">
                        <Button
                          ...
                          onClick={() =>
                            onEdit(
                              member.id,
                              member.refSamityId || member.centralSamityId,
                              member.samityCode || member.centralSamityCode,
                              member.memberName,
                              member.memberAdmissionDate,
                              member.samityRegistrationDate,
                              member.samitySignatoryPerson || member.signatoryPersonNameBangla,
                              member.samityDetailsAddress || member.centralSamityDetailsAddress,
                              member.documents,
                            )
                          }
                        >
...
export default CenNatMemberRegistration;
