...
const CenMemberDetailsCorrection = (props) => {
  ...
  const sendCorrectionMemberList = async (id) => {
    try {
      await axios.patch(memberInfoCorrectionRequest + id, '', config);
      allMemberShow();
      NotificationManager.success('আবেদনটি সফলভাবে জমা দেওয়া হয়েছে, দয়া করে অনুমোদনের জন্য অপেক্ষা করুন।', '', 5000);
    } catch (error) {
      errorHandler(error);
    }
  };
  ...
  const onEdit = async (data) => {
    ...
    setSamityInfo({
      ...samityInfo,
      id: data.centralSamityId,
      samityId: samityPerId,
      samityCode: data.centralSamityCode,
      samityName: data.memberName,
      samityRegistrationDate: dateFormat(data.memberAdmissionDate),
      memberNameBangla: data.memberName,
      samityDetailsAddress: data.centralSamityDetailsAddress,
      samitySignatoryPerson: data.signatoryPersonNameBangla,
      mobile: data.mobile,
    });
    // image set start
    if (data.documents) {
  ...
  //       if (result.isConfirmed) {
  //         if (getCorrectionMemData.data.membersInfo.length == 1) {
  //           axios.delete(deleteApplication + getCorrectionMemData.id, config)
  //             .then((response) => {
  //               if (response.status == 200) {
  //                 Swal.fire(
  //                   "বাতিল হয়েছে!",
  //                   "আপনার মেম্বার বাতিল করা হয়েছে.",
  //                   "success"
  //                 );
  //                 allMemberShow();
  //               } else {
  ...
  //           let list = { ...getCorrectionMemData };
  //           list.data.membersInfo.splice(index, 1);
  //           axios.put(InsertMemCorrectionData + "/" + getCorrectionMemData.id, { ...list, serviceName: "member_information_correction" }, config)
  //             .then((response) => {
  //               if (response.status == 200) {
  //                 Swal.fire(
  //                   "বাতিল হয়েছে!",
  ...
  let onSubmitData = async (e, isDelete, member) => {
          ...
          membersInfo: [
            {
              ...
              refSamityId: member.memberBasicInfo.centralSamityId,
              memberAdmissionDate: dateFormat(member.memberBasicInfo.memberAdmissionDate),
              address: member.memberBasicInfo.centralSamityDetailsAddress,
              samitySignatoryPerson: member.memberBasicInfo.signatoryPersonNameBangla,
    ...
    //     if (isDelete) {
    //       // delete update part
    //       getCorrectionMemData.data.membersInfo.push(
    //         deletePayload.data.membersInfo[0]
    //       );
    //       insertCorectionData = await axios.put(
    //         InsertMemCorrectionData + "/" + getCorrectionMemData.id,
    //         { ...getCorrectionMemData, serviceName: "member_information_correction" },
    ...
    //       // update part
    //       if (memberIndex != null) {
    //         getCorrectionMemData.data.membersInfo[memberIndex] =
    //           correctionPayload.data.membersInfo[0];
    //         insertCorectionData = await axios.put(
    //           InsertMemCorrectionData + "/" + getCorrectionMemData.id,
    //           { ...getCorrectionMemData, serviceName: "member_information_correction" },
    //           config
    ...
    //   } else {
    //     if (isDelete) {
    //       insertCorectionData = await axios.post(
    //         InsertMemCorrectionData,
    //         deletePayload,
    //         config
    //       );
                  ...
                  <>
                    ...
                    {/* <Grid key={i} item md={4} xs={12}>
                          <Card sx={{ maxWidth: 345 }}>
                            ...
                            <Card sx={{ height: "250px", border: element?.imageError ? '1px solid red' : "" }}>
                              {
                                element?.base64Image ? (
                                  <>
                                    <ZoomSelectImage src={element?.base64Image} xs={{ width: "100%", height: "100%" }} key={i} type={element?.mimeType === "application/pdf" ? "pdf" : ""} />

                                  </>
                                ) : (
                                  <ZoomImage src={element?.fileNameUrl} imageStyle={{ width: "100%", height: "100%" }} divStyle={{ display: 'flex', justifyContent: 'center' }} key={i} type={imageType(element?.fileName)} />
                                )
                              }

                            </Card>
                            <CardContent sx={{ flex: "1 0 auto" }}>
                              <span style={{ color: 'red', display: 'block' }}> {element?.imageError}</span>
                              ...
                              <Button onClick={(e) => removeSelectedImageletter(e, i)}
                                variant="outlined" color="error"
                                component="span"
                                startIcon={<HighlightOffSharpIcon sx={{ color: 'red' }} />}
                                sx={{ ml: '5px', display: element?.base64Image ? '' : 'none' }}
                              >
                                বাতিল
                              </Button>
                            </CardContent>
      ...
      <Grid container className="btn-container">
        {isAddMember ? (
          loadingDataSaveUpdate ? (
            ...
            <Tooltip title={update || memberCorrection ? 'হালনাগাদ করুন' : 'সংরক্ষন করুন'}>
              <Button
                disabled={dataEmpty}
        ...
        {getCorrectionMemData?.length != 0 && isAddMember == false ? (
          <Button
              ...
              <TableBody>
                {showAllMember?.map((member, index) => (
                  <TableRow key={index}>
                    ...
                    <StyledTableCell>
                      <Tooltip title={<div className="tooltip-title">{member?.memberBasicInfo?.memberName}</div>} arrow>
                ...
                <TableBody>
                  {getCorrectionMemData?.data?.membersInfo?.map((member, index) => (
                    <TableRow key={index}>
                      ...
                      <StyledTableCell>
                        <Tooltip title={<div className="tooltip-title">{member.address}</div>}>
                      ...
                      <StyledTableCell>
                        <Tooltip title={<div className="tooltip-title">{member.mobile}</div>}>
