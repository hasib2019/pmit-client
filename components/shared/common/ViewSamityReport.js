...
const ViewSamityReport = () => {
  ...
  useEffect(() => {
    samityReport(getSamityId);
  }, [getSamityId]);

  const samityReport = async (getSamityId) => {
    try {
      setLoadData(true);
      const samityData = await axios.get(ApproveSamityReportApi + getSamityId, config);
    ...
    <>
      {loadData ? (
        ...
        >
          ...
          >
            <Typography sx={{ fontSize: '18px' }}>সমিতির বিস্তারিত তথ্য</Typography>
            <Button
              variant="contained"
...
export default ViewSamityReport;
